...
func (c *SSHCapture) Start(ctx context.Context) {
	...
	// fine. Harmless on single interfaces, but only added for "any".
	linkOpt := ""
	if c.config.Interface == "any" {
		linkOpt = "-y LINUX_SLL "
	}
	tcpdumpCmd := fmt.Sprintf("sudo tcpdump -U %s-w - -i %s -s 0 '%s'", linkOpt, c.config.Interface, bpfFilter)
	...
	// Read and process pcap stream (the local save file is created inside, once
	// the stream's link type is known).
...
// createPcapFile creates a new pcap file with timestamp. The header is written
...
func (c *SSHCapture) processPcapStream(ctx context.Context, reader io.Reader) {
	...
	// Now that the link type is known, create the local save file with a matching
	// header (Ethernet for one interface, SLL/SLL2 for `-i any`).
	if c.enablePcap {
		if err := c.createPcapFile(linkType); err != nil {

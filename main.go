...
func main() {
	...
	if replayOnlyMode {
		...
		for _, pwt := range allPackets {
			pkt := pwt.Info
			srcLabel := pkt.SrcName
			if srcLabel == "" {
				srcLabel = pkt.SrcIP
			}
			dstLabel := pkt.DstName
			if dstLabel == "" {
				dstLabel = pkt.DstIP
			}
			graphMgr.AddOrUpdateNode(pkt.SrcIP, srcLabel, pkt.Length)
			graphMgr.AddOrUpdateNode(pkt.DstIP, dstLabel, pkt.Length)
			graphMgr.AddOrUpdateEdge(pkt.SrcIP, pkt.DstIP, pkt.Protocol, pkt.Length)
			graphMgr.AddPortObservation(pkt.SrcIP, pkt.DstIP, pkt.SrcPort, pkt.DstPort)
		...
		go func() {
			for {
				...
				case <-ctx.Done():
					return
				case pkt := <-packetChan:
					// Resolve hostnames asynchronously. Endpoints without a
					// resolvable IP (e.g. L2 topology nodes) carry a friendly
					// name on the packet, which takes precedence over DNS.

...
(function () {
    ...
    class GLNetwork {
        ...
        constructor(container, data, options) {
            ...
            // When set ({min,max}), node sizes scale against this fixed value
            // domain instead of the current view's min/max. Replay mode uses
            // the full capture's range so nodes shrink/grow while scrubbing.
            this.fixedValueRange = null;
        ...
        _recomputeSizes() {
            ...
            if (this.fixedValueRange) {
                minV = this.fixedValueRange.min;
                maxV = this.fixedValueRange.max;
                any = true;
            } else {
                for (const id in this.body.nodes) {
            ...
            for (const id in this.body.nodes) {
                ...
                const f = Math.max(0, Math.min(1, (o.value - minV) / (maxV - minV)));
                o.size = 20 + 10 * f;
            }
        }

        // Fix the value domain used for node sizing (null restores per-view
        // dynamic scaling). See fixedValueRange.
        setValueRange(min, max) {
            this.fixedValueRange = (min == null || max == null) ? null : { min: min, max: max };
            this._recomputeSizes();
            this._nodesDirty = true;
            this.redraw();
        }

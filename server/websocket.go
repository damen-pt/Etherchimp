...
type Client struct {
	...
	// Per-client layout engine, used only for timeline-mode views (each timeline
	// client scrubs its own window). Live views share one engine per config
	// bucket (hub.sharedEngines, viewcache.go).
...
type Hub struct {
	...
	// lastRoleCommit paces CommitRoles (~1s; the role damping window is 2s).
	lastRoleCommit time.Time
	// sharedEngines holds one layout engine per distinct view-config bucket so
	// identically configured clients share a single BuildView per tick (hub
	// goroutine only; see viewcache.go).
	sharedEngines map[string]*sharedLayoutEngine
...
func NewHub(graphMgr *graph.Manager, overrides *graph.OverrideStore, db *store.Store) *Hub {
	return &Hub{
		...
		db:         db,
		sharedEngines: make(map[string]*sharedLayoutEngine),
	}
}
...
func (h *Hub) tick() {
	...
	for client := range h.clients {
		cfg := client.snapshotCfg()
		mode := cfg.LayoutMode
		if client.peekNeedsFull() {
			anyNeedsFull = true
		}
		if client.timelineDirty() {
			anyTimelineDirty = true
		}
		if mode == "subnet" {
			subnetActive = true
		}
		// Raw and timeline clients keep per-client layout engines; live clients
		// share one engine per config bucket (see viewcache.go).
		if cfg.Raw || client.hasTimeline() {
			if client.layout.Unsettled(map[string]bool{mode: true}) {
				anyLayoutBusy = true
			}
		} else if h.sharedEngineUnsettled(viewConfigKey(cfg), mode) {
			anyLayoutBusy = true
		}
	}
	...
	// Commit damped role classifications at ~1s cadence (kept off SnapshotRaw's
	// path so the per-tick snapshot copy stays read-locked — see CommitRoles).
	if time.Since(h.lastRoleCommit) >= time.Second {
		h.lastRoleCommit = time.Now()
		h.graphMgr.CommitRoles()
	}
	...
	// Live views built this tick, shared across clients with identical view
	// configs (see viewcache.go); deltas remain per-client.
	bucketViews := make(map[string]graph.ViewSnapshot)
	...
	for client := range h.clients {
		...
		func() {
			...
			clientFlows := flows
			tl, tlActive := client.timelineParams()
			if tlActive {
			...
			cfg := client.snapshotCfg()
			var view graph.ViewSnapshot
			if tlActive {
				// Timeline views are per-client (each scrubs its own window).
				view = graph.BuildView(clientRaw, cfg, client.layout, h.overrides, pins, vlanByIP)
			} else {
				// Live clients share one BuildView per config bucket: the layout
				// is a config property (hidden nodes must not spread visible
				// ones), so identical configs get identical physics for free.
				key := viewConfigKey(cfg)
				v, ok := bucketViews[key]
				if !ok {
					v = graph.BuildView(clientRaw, cfg, h.sharedEngineFor(key), h.overrides, pins, vlanByIP)
					bucketViews[key] = v
				}
				view = v
			}
...
// peekNeedsFull reports whether a full view is pending, without clearing it.
func (c *Client) peekNeedsFull() bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.needsFull
}

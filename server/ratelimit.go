...
// TrustProxyHeaders controls whether X-Forwarded-For / X-Real-IP are honored when
// identifying the client. These headers are client-spoofable, so trusting them on a
// directly-exposed server lets a client forge a fresh IP per request — bypassing the
// per-IP rate limit and bloating the client table. Default false (secure); enable
// only when running behind a trusted reverse proxy that sets them.
var TrustProxyHeaders = false

// GetClientIP extracts the client IP from the request.
func GetClientIP(r *http.Request) string {
	if TrustProxyHeaders {
		// Check X-Forwarded-For header (only trustworthy behind a known proxy).
	...
	// Authoritative source: the transport peer address (strip port).
	ip := r.RemoteAddr

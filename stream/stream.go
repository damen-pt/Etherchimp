...
var (
	httpReqRe  = regexp.MustCompile(`^(GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH)\s+(\S+)\s+HTTP`)
	smtpFromRe = regexp.MustCompile(`MAIL FROM:<([^>]+)>`)
)
...
const (
	...
	ProtocolInfluxDB StreamProtocol = "InfluxDB"
...
type StreamDetail struct {
	...
	// RequestHex/ResponseHex are preformatted hex dumps of the reassembled
	// payloads, so the client's raw tab renders them verbatim.
...
func generateStreamID(srcIP string, srcPort uint16, dstIP string, dstPort uint16, streamType StreamType) string {
	// Normalize direction for consistent ID (lower IP:port first). Built with
	// strconv/concatenation rather than fmt.Sprintf since this runs per packet.
	...
	return string(streamType) + "-" + src + "-" + dst
}
...
func extractHTTPSummary(stream *Stream) string {
	...
	if len(lines) > 0 {
		...
		if matches := httpReqRe.FindStringSubmatch(firstLine); len(matches) >= 3 {
			method := matches[1]
			path := matches[2]
			if len(path) > 50 {
				path = path[:50] + "..."
...
func extractSMTPSummary(stream *Stream) string {
	...
	// Look for MAIL FROM
	if matches := smtpFromRe.FindStringSubmatch(data); len(matches) >= 2 {
...
func (m *Manager) GetStream(id string) (*StreamDetail, error) {
	...
	if len(stream.RequestData) > 0 {
		detail.RequestHex = capture.HexDumpText(stream.RequestData, 4096)
	}
	if len(stream.ResponseData) > 0 {
		detail.ResponseHex = capture.HexDumpText(stream.ResponseData, 4096)
	}
